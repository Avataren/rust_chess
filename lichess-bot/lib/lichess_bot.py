...
def play_game(li: lichess.Lichess,
    ...
    with li.get_game_stream(game_id) as response:
        ...
        with engine_wrapper.create_engine(config, game) as engine:
            ...
            prior_game = None
            board = chess.Board()
            was_mating: list[bool] = [False]  # tracks borg-taunt state across moves
                ...
                try:
                    ...
                    if u_type == "chatLine":
                        ...
                        if not is_game_over(game) and is_engine_move(game, prior_game, board):
                            ...
                            move_attempted = True
                            scores_before = len(engine.scores)
                            ...
                            if len(engine.scores) > scores_before:
                                say_borg_taunt_if_mating(conversation, engine, was_mating)
                            else:
                                was_mating[0] = False
...
def say_borg_taunt_if_mating(conversation: Conversation, engine: engine_wrapper.EngineWrapper,
                              was_mating: list[bool]) -> None:
    """Taunt once when we first detect a forced mate. Stay quiet during the execution."""
    if not engine.scores:
        was_mating[0] = False
        return
    mate_in = engine.scores[-1].relative.mate()
    if mate_in is not None and mate_in >= 2:
        if not was_mating[0]:
            taunt = BORG_TAUNTS[len(engine.scores) % len(BORG_TAUNTS)]
            conversation.send_message("player", taunt)
            conversation.send_message("spectator", taunt)
        was_mating[0] = True
    else:
        was_mating[0] = False
